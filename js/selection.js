generateTeams = (selectedTeam) => {
  ...
  let hexRadius = d3.min([
    width / ((hexColumns + 0.5) * Math.sqrt(3)),
    height / ((hexRows + 1 / 3) * 1.5),
  ]);
  ...
  for (let text in textArray) {
    svg
      .append("circle")
      .attr("cx", textArray[text][1])
      .attr("cy", textArray[text][2])
      .attr("r", 7)
      .style("fill", `${textArray[text][4]}`);
    svg
      .append("text")
      .attr("x", textArray[text][3])
      .attr("y", textArray[text][2])
